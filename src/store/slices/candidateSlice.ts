...
const candidateSlice = createSlice({
  ...
  reducers: {
    ...
    addSavedJob: (state, action: PayloadAction<SavedJob>) => {
      const exists = state.savedJobs.some((j) => j.jobId === action.payload.jobId)
      if (!exists) {
    ...
    updateBuildMyCvPersonalDetails: (state, action: PayloadAction<Partial<BuildMyCvPersonalDetails>>) => {
      state.buildMyCv.personalDetails = { ...state.buildMyCv.personalDetails, ...action.payload }
    },

    updateBuildMyCvCareerHistory: (state, action: PayloadAction<CandidateExperience[]>) => {
    ...
    updateBuildMyCvEducation: (state, action: PayloadAction<BuildMyCvEducationSection>) => {
      state.buildMyCv.education = action.payload
    },

    updateBuildMyCvLanguages: (state, action: PayloadAction<CandidateLanguage[]>) => {
